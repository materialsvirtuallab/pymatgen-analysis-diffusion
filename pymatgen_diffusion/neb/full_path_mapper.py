...
class FullPathMapper(MSONable):
    ...
    (between sites) are edges. The migration graph object used
    ...
    def __init__(
        ...
        """
        Construct the FullPathMapper object using a base structure and
        corresponding occupued mobile sites. A structure graph is
        generated by connecting all mobile sites within
        max_path_length distance of each other. The sites are decorated
        with Migration graph objects and then grouped together based
        ...
        Args:
            ...
            sites_structure (Structure): Structure of sites (corresponding
                to the base framework) that the mobile specie occupies.
            max_hop_length (float): Maximum distance between atoms
        ...
        self.base_with_sites = Structure.from_sites(sites_list)

        self.max_hop_length = max_hop_length
        self.symprec = symprec
        self.a = SpacegroupAnalyzer(self.base_with_sites, symprec=self.symprec)
        ...
        self.s_graph = StructureGraph.with_local_env_strategy(
            self.sites_structure,
            MinimumDistanceNN(cutoff=max_hop_length, get_all_sites=True),
    ...
    def _get_pos_and_migration_path(self, u, v, w):
        ...
        i_site = self.sites_structure.sites[u]
        e_site = PeriodicSite(
            self.sites_structure.sites[v].species,
            self.sites_structure.sites[v].frac_coords + np.array(edge["to_jimage"]),
            lattice=self.sites_structure.lattice,
        )
        ...
        edge["ipos_cart"] = np.dot(
            i_site.frac_coords, self.sites_structure.lattice.matrix
        )
        edge["epos_cart"] = np.dot(
            e_site.frac_coords, self.sites_structure.lattice.matrix
        )
    ...
    def get_path(self, max_val=100000):
        """
        obtain a pathway through the base structure using hops that are in the current graph
        Basic idea:
        ...
        if len(self.unique_hops) != len(self.unique_hops):
            logger.error(
                f"There are {len(self.unique_hops)} SC hops but {len(self.unique_hops)} UC hops"
            )
            ...
            for (idx1, jimage1), (idx2, jimage2) in zip(path[:-1], path[1:]):
                ...
                for k, tmp_d in all_edge_data:
                    if tmp_d["to_jimage"] in {tuple(image_diff), tuple(-image_diff)}:
                        ...
                        found_ += 1
                if found_ != 1:
                    raise RuntimeError("More than one edge mathched in original graph.")
            yield u, path_hops
    ...
    def modify_path(self, paths):
        """
        This takes the results of get_path() and turn them into an ordered path:
    ...
    def populate_edges_from_dict(self, unique_hop_dict):
